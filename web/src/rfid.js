...
function listhardware() {
  ...
  if (isOfficialBoard) {
    document.getElementById("readertype").value =6;
...
function uncommited() {
  ...
  document.getElementById("commit").innerHTML = "<h5>有未同步修改，请点击确认.</h5>";
    ...
    $("body").load("esprfid.htm #progresscontent", function (responseTxt, statusTxt, xhr) {
        ...
        $(".progress-bar").css("width", i + "%").attr("aria-valuenow", i).html(i + "%");
                ...
                if (i === 101) {
                    ...
                    document.getElementById("updateprog").innerHTML = "完成";
...
function handleMQTT() {
  if (document.querySelector("input[name=\"mqttenabled\"]:checked").value === "0") {
    $("#mqtten").slideUp();
  } else {
    $("#mqtten").slideDown();
    $("#mqtten").show();
...
function listSSID(obj) {
  ...
  for (var i = 0; i < obj.list.length; i++) {
    ...
    opt.bssidvalue = obj.list[i].bssid;
        opt.innerHTML = "相对信号强度 :" + percentage + " % ,  热点名称 :  " + obj.list[i].ssid;
...
function getnextpage(mode) {
  if (!backupstarted) {
    document.getElementById("loadpages").innerHTML = " " + page + "/" + haspages;
  }
...
function backupset() {
  ...
  dlAnchorElem.setAttribute("download", "QRFID-settings.json");
  dlAnchorElem.click();
}
...
function piccBackup(obj) {
  ...
  dlAnchorElem.setAttribute("href", dataStr);
  dlAnchorElem.setAttribute("download", "QRFID-users.json");
  dlAnchorElem.click();
  backupstarted = false;
}
...
function restore1by1(i, len, data) {
  ...
  if (slot === len) {
    ...
    document.getElementById("dynamic").innerHTML = "完成";
  ...
  if ("files" in input) {
    if (input.files.length === 0) {
      ...
      reader.onload = function() {
        ...
        if (json.type === "RFID-data-backup") {
          var x = confirm("请确认文件格式是否有效?");
          if (x) {
            recordstore = json.list.length;
            data = json.list;
            restorestarted = true;
            $("#restoremodal").modal({
              backdrop: "static",
            ...
            restore1by1(slot, recordstore, data);
          }
        }
      };
      reader.readAsText(input.files[0]);
      ...
      ft = window.FooTable.init("#usertable", {
        ...
        editing: {
          ...
          editRow: function(row) {
            ...
            if (values.acctype === "通行") {
              acctypefinder = 1; }
            else if (values.acctype === "关注") {
              acctypefinder = 10; }
            else if (values.acctype === "禁止") {
              acctypefinder = 2; }
            else if (values.acctype === "限制") {
              acctypefinder = 3; }
            else if (values.acctype === "未配置") {
              acctypefinder = 0; }
            else if (values.acctype === "管理") {
              acctypefinder = 99;} 

            $editor.find("#uid").val(values.uid);
            $editor.find("#picctype").val(values.picctpye);
            ...
            $modal.data("row", row);
            $editorTitle.text("编辑用户 ： " + values.username);
            $modal.modal("show");
          },
          deleteRow: function(row) {
            var uid = row.value.uid;
            var username = row.value.username;
            if (confirm("删除用户 :  " + username + " [ " + uid +  " ]，请确认操作 ?")) {
    ...
    $editor.on("submit", function(e) {
      ...
      var row = $modal.data("row"),
        values = {
          ...
          picctpye: $editor.find("#picctype").val(),
      ...
      datatosend.picctpye = $editor.find("#picctpye").val();
...
function socketMessageListener(evt) {
  ...
  if (obj.hasOwnProperty("command")) {
    switch (obj.command) {
      ...
      case "configfile":
        config = obj;
        if (!('wifipin' in config.hardware)) config.hardware.wifipin = 2;
...
window.FooTable.MyFiltering = window.FooTable.Filtering.extend({
  construct: function(instance) {
    ...
    this.acctypes = ["1", "2", "3","10","99"];
    this.acctypesstr = ["通行","禁止", "限制", "关注", "管理"];
    this.def = "权限类型";
    this.$acctype = null;
  },
  $create: function() {
...
function getLatestReleaseInfo() {
  ...
  $.getJSON("https://api.github.com/repos/Qful/RFID/releases/latest").done(function(release) {
    ...
    var timeAgo;
    if (dateDiff < oneDay) {
            timeAgo = (dateDiff / oneHour).toFixed(1) + " 小时前";
    } else {
            timeAgo = (dateDiff / oneDay).toFixed(1) + " 天前";
    }

        var releaseInfo =  release.name + "   发布于 " + timeAgo + " ，已被下载" + downloadCount.toLocaleString() + " 次";
    ...
    }).error(function () { $("#onlineupdate").html("<h5>无法获取固件信息，请检查网络是否可正常访问?</h5>"); });
}
