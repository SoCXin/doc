...
* 关键词：``RISC-V3A`` ``120MHz`` ``USB 3.0`` ``HSPI`` ``SerDes`` ``Ethernet`` ``EMMC``
...
* :ref:`wch_serdes` 可直接驱动光纤，支持网线（仅使用1组差分线）传输90米，支持1.25Gbps高速差分信号通讯
...
:ref:`ch569` 目前仅有一个封装规格，而 :ref:`ch565` 具有QFN68/QFN40
...
^^^^^^^^^^^
 ...
 :ref:`ch565` 相较于 :ref:`ch569` 多了DVP接口，去掉了 :ref:`wch_hspi` ，甚至在更小QFN40上去掉了以太网和调试端口。
