...
//检查是否立即插入了网线
void check_NET_Linker(void)
{
	static u8 net_link_status = 1;
	u8 status  = getNetLink();
	
	if(net_link_status == 0 && status == 1)
		dhcp_retry_count = MAX_DHCP_RETRY;
