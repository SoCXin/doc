...
void timer3_init(void);																			/*STM32定时器2初始化*/
void timer3_isr(void);																			/*定时器中断执行的函数*/
...
uint8_t Read_W5500_1Byte(uint16_t reg);//从寄存器里面读取数据

/*W5500基本配置相关函数*/
void reset_w5500(void);																			/*硬复位W5500*/
void set_w5500_mac(void);																		/*配置W5500的MAC地址*/
void set_w5500_ip(uint8_t ip_from);																			/*配置W5500的IP地址*/
char*get_w5500_ip(void);																				//获取IP地址
